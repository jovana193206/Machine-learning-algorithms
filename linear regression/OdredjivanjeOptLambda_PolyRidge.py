...
lambdas = np.linspace(1e-3, 20, 20)
J = np.zeros((20,))
scores = np.zeros((20,))

#Za svako lamda iz lamdbdas istreniraj teta i izracunaj J na validacionom skupu
#Optimalan stepen polinoma je 2
i = 0
for l in lambdas:
    print('lambda = ', l)
    x_poly = polyFeatures(x_train, 2)
    teta = fit(x_poly, y_train, l)
    x_valid_poly = polyFeatures(x_valid, 2)
    y_pred = predict(x_valid_poly)
    #Nadji srednju kvadratnu gresku za stepen d
    J[i] = (1/2)*np.sum(np.square(np.subtract(y_pred, y_valid))) 
    scores[i] = 1000000/J[i]
    print('J = ', J[i])
    i = i + 1
...
plt.xlabel("lambda")
plt.ylabel("Skor")
plt.plot(lambdas, scores, color="navy")
plt.show()

print('Optimalan parametar ridge regularizacije (lambda): ', lambdas[np.argmax(scores)])
