...
for i in range(len(ans_size)):
    s = ans_size[i]
    model2 = RandomForestClassifier(n_estimators=s, max_depth=2, bootstrap=True)
    model2.fit(x_train, y_train)
    valid_accuracy2[i] = model2.score(x_valid, y_valid)
    train_accuracy2[i]= model2.score(x_train, y_train)
    model4 = RandomForestClassifier(n_estimators=s, max_depth=4, bootstrap=True)
    model4.fit(x_train, y_train)
    valid_accuracy4[i] = model4.score(x_valid, y_valid)
    train_accuracy4[i]= model4.score(x_train, y_train)
    model5 = RandomForestClassifier(n_estimators=s, max_depth=5, bootstrap=True)
