...
for i in range(len(sigmas)):
    ...
    accuracy_arr[i] =  accuracy(y_pred, y_valid)
     
plt.title("Validaciona kriva")
plt.xlabel("sigma")
plt.ylabel("Tacnost [%]")
plt.plot(sigmas, accuracy_arr, color="navy")
plt.show()
sigma_opt = sigmas[np.argmax(accuracy_arr)]
print('Optimalan parametar sigma: ', sigma_opt)      
        
        
for j in range(len(Cs)):
    C = Cs[j]
    #istreniraj model - nadji w i b
    w, b = QP(x_train, y_train, C, sigma_opt)
    y_pred = predict(x_valid, w, b)
    #nadji tacnost na validacionom skupu
...
plt.show()
C_opt = Cs[np.argmax(accuracy_arr)]
print('Optimalan parametar C: ', C_opt)   
